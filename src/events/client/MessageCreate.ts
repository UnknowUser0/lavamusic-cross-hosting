...
export default class MessageCreate extends Event {
	...
	public async run(message: Message): Promise<any> {
		...
		// Cegah multiple responses dengan hanya menjalankan di satu cluster utama
		if (ClusterClient.getInfo().FIRST_SHARD_ID !== 0) return;
		// return command.run(this.client, new Context(message, args, this.client));
